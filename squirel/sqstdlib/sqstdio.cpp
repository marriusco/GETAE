...
//basic API
#ifndef _UNICODE
    extern char*   sq_includes[256];
    typedef char   RCHAR;
    #define Tfind   _findfirst
#else
    extern wchar*    sq_includes[256];
    typedef short   RCHAR;
    #define Tfind   _wfindfirst
#endif //
...
SQFILE sqstd_fopen(const SQChar *filename ,const SQChar *mode)
{
    ...
    long    hFile    = Tfind( filename, &c_file );
    if(hFile ==-1)
    {
        for(int i=0; sq_includes[i]; i++)
        {
            ...
            {
                #ifndef _UNICODE
                sprintf(loco,"%s%s",sq_includes[i],filename);
                #else
                _stprintf(loco,L"%s%s",sq_includes[i],filename);
                #endif //

                hFile    = Tfind( loco, &c_file );
                if(hFile!=-1)
                {
                    _findclose( hFile );
                    break;
    ...
    else
    {
        ...
        #else
            wcscpy(loco,filename);
        #endif
        _findclose( hFile );
    }

#ifndef _UNICODE
	return (SQFILE)fopen(loco,mode);
#else
	return (SQFILE)_wfopen(loco,mode);
#endif
}
