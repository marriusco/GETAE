...
// mco array
struct ScriptCharArray
{
  
  char     s[256];   //maxim 256
  operator char * () { return s; }
  operator void * () { return (void *)s; }
  const char * operator = (const char * ps) {
      char* pd = s;
      while(*ps)
      {
        *pd++=*ps++;
...
struct TypeInfo<unsigned int> {
  ...
  enum {TypeID=VAR_TYPE_INT,Size=sizeof(unsigned int)};
  operator ScriptVarType() { return ScriptVarType(TypeID); }
};
...
struct TypeInfo<unsigned char> {
  const SQChar * typeName;
  TypeInfo() : typeName(_T("uchar")) {}
  enum {TypeID=VAR_TYPE_CHAR,Size=sizeof(unsigned char)};
  operator ScriptVarType() { return ScriptVarType(TypeID); }
};
...
struct TypeInfo<unsigned long> {
  const SQChar * typeName;
  TypeInfo() : typeName(_T("ulong")) {}
  enum {TypeID=VAR_TYPE_INT,Size=sizeof(unsigned long)};
  operator ScriptVarType() { return ScriptVarType(TypeID); }
};
...
template<>//mco array
struct TypeInfo<ScriptCharArray> {
  const SQChar * typeName;
  TypeInfo() : typeName(_T("ScriptCharArray")) {}
  enum {TypeID=VAR_TYPE_ASTRING,Size=sizeof(ScriptCharArray)};
  operator ScriptVarType() { return ScriptVarType(TypeID); }
};
...
struct VarRef {
  ...
  VarRef(void * _offsetOrAddrOrConst, 
        ScriptVarType _type, 
...
struct ClassType {
  ...
  static void copy(T * dst,T * src) {
    *dst = *src; //mcoout
    ////memcpy((void*)dst, (void*)src, sizeof(*dst));//mco iface in
  } // copy
};
...
//MCO
template<typename T>
void RegisterInstanceVariableStorage(SquirrelObject & so,SQUserPointer classType,T * var,const SQChar * scriptVarName,VarAccessType access=VAR_ACCESS_READ_WRITE) {
    VarRef * pvr = createVarRef(so,scriptVarName);
    void * offsetOrAddrOrConst = (void *)var; // var must be passed in as &obj->var, where obj = 0 (the address is the offset), or as static/global address.
    *pvr = VarRef(offsetOrAddrOrConst,TypeInfo<T>(),classType,ClassTypeStorage<T>::getCopyFunc(),sizeof(*var),access,TypeInfo<T>().typeName);
    createInstanceSetGetHandlers(so);
} // RegisterInstanceVariable
...
inline BOOL CreateCopyInstance(const SQChar * className,const T & classToCopy) {
  ...
  if (!up) return FALSE;
  T * newClass = (T *)up;
  *newClass = classToCopy; // <TODO> Optimized version that uses the copy constructor. mcoout
  ////memcpy(newClass, &classToCopy, sizeof(*newClass));//mco iface in
  return TRUE;
} // CreateCopyInstance

////MCO
template<typename T>
inline BOOL CreateCopyStorage(const SQChar * className,const T & classToCopy) {
  HSQUIRRELVM v = SquirrelVM::GetVMPtr();
  if (!CreateConstructNativeClassInstance(v,className)) {
    return FALSE;
  } // if
  SQUserPointer up=0;
  sq_getinstanceup(v,-1,&up,ClassTypeStorage<T>::type());
  if (!up) return FALSE;
  T * newClass = (T *)up;
  memcpy(newClass, &classToCopy, sizeof(*newClass));
  return TRUE;
} // CreateCopyInstance
...
// Get an instance of type T from the stack at idx (for function calls).
template<typename T>
T * GetInstanceStorage(HSQUIRRELVM v,SQInteger idx) {
  SQUserPointer up=0;
  sq_getinstanceup(v,idx,&up,ClassTypeStorage<T>::type());
  if (!up) throw SquirrelError(_T("GetInstance: Invalid argument type\r\n"));
  return (T *)up;
} // GetInstance
...
#define DECLARE_INSTANCE_TYPE_NAME_(TYPE,NAME) namespace SqPlus { \
  ...
  inline TYPE * Get(TypeWrapper<TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE>(v,idx); } \
\
 inline bool	Match(TypeWrapper<TYPE##TYPE &>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE>(v,idx) != NULL; } \
  inline bool	Match(TypeWrapper<TYPE##TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE>(v,idx) != NULL; } \
  inline TYPE##TYPE & Get(TypeWrapper<TYPE##TYPE &>,HSQUIRRELVM v,int idx) { return *GetInstance<TYPE>(v,idx); } \
  inline TYPE##TYPE * Get(TypeWrapper<TYPE##TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstance<TYPE>(v,idx); } \
  template<> \
...
//mco template instances
#define DECLARE_TEMPLATE_TYPE(TEMPLATE, TYPE) typedef TEMPLATE<TYPE> TEMPLATE##TYPE;\
                                                DECLARE_INSTANCE_TYPE_NAME_(TEMPLATE##TYPE,TEMPLATE##TYPE)
...
////MCO
#define DECLARE_STORAGE_TYPE_NAME_(TYPE,NAME,SQ_TYPE) namespace SqPlus { \
    typedef const  TYPE TYPE##TYPE;\
    inline const SQChar * GetTypeName(const TYPE & n)            { return _T(#NAME); } \
    inline void   Push(HSQUIRRELVM v,TYPE * value)                 { if (!CreateNativeClassInstance(v,GetTypeName(*value),value,0)) throw SquirrelError(_T("Push(): could not create INSTANCE (check registration name)\r\n")); } \
    inline void   Push(HSQUIRRELVM v,TYPE & value)                 { if (!CreateCopyStorage(GetTypeName(value),value)) throw SquirrelError(_T("Push(): could not create INSTANCE copy (check registration name)\r\n")); } \
    inline bool	Match(TypeWrapper<TYPE &>,HSQUIRRELVM v,int idx) { return  GetInstanceStorage<TYPE>(v,idx) != NULL; } \
    inline bool	Match(TypeWrapper<TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstanceStorage<TYPE>(v,idx) != NULL; } \
    inline TYPE & Get(TypeWrapper<TYPE &>,HSQUIRRELVM v,int idx) { return *GetInstanceStorage<TYPE>(v,idx); } \
    inline TYPE * Get(TypeWrapper<TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstanceStorage<TYPE>(v,idx); } \
    \
    inline bool	Match(TypeWrapper<TYPE##TYPE &>,HSQUIRRELVM v,int idx) { return  GetInstanceStorage<TYPE>(v,idx) != NULL; } \
    inline bool	Match(TypeWrapper<TYPE##TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstanceStorage<TYPE>(v,idx) != NULL; } \
    inline TYPE##TYPE & Get(TypeWrapper<TYPE##TYPE &>,HSQUIRRELVM v,int idx) { return *GetInstanceStorage<TYPE>(v,idx); } \
    inline TYPE##TYPE * Get(TypeWrapper<TYPE##TYPE *>,HSQUIRRELVM v,int idx) { return  GetInstanceStorage<TYPE>(v,idx); } \
    template<> \
struct TypeInfo<TYPE> { \
    const SQChar * typeName; \
    TypeInfo() : typeName(_T(#NAME)) {} \
    enum {TypeID=(ScriptVarType)SQ_TYPE,Size=sizeof(TYPE)}; \
    operator ScriptVarType() { return ScriptVarType(TypeID); } \
}; \
} // nameSpace SqPlus

////MCO
#define DECLARE_STORAGE_TYPE(TYPE,SQ_TYPE) DECLARE_STORAGE_TYPE_NAME_(TYPE,TYPE,SQ_TYPE)

#define DECLARE_ARRAY_TYPE(TYPE,SZ,SQ_TYPE)  typedef TYPE TYPE##SZ[SZ]; \
                                              DECLARE_STORAGE_TYPE_NAME_(TYPE##SZ,TYPE#SZ,SQ_TYPE)


//typedef char UC5[5];
//DECLARE_STORAGE_TYPE(UC5);
//DECLARE_STORAGE_TYPE_SZ(char,5);
...
//::::inline bool	Match(TypeWrapper<unsigned char>,HSQUIRRELVM v, int idx) { return sq_gettype(v,idx) == OT_BYTE; }
...
#ifdef _VC6CRAP
    template<class T> inline void GetRet(TypeWrapper<void>,HSQUIRRELVM v,int idx) { sq_pop(v,2); }
#else
...
//        try{delete self;}catch(...){}       \   

#define SQ_DELETE_CLASS(CLASSTYPE) if (up) { \
        CLASSTYPE * self = (CLASSTYPE *)up; \
        delete self;                        \
} return 0  ;
...
struct SQClassDef {
    ...
    template<typename Func>  SQClassDef & staticFuncVarArgs(Func pfunc,const SQChar * name,const SQChar * typeMask=_T("*")) 
    {
    ...
    template<typename Func>   SQClassDef & staticFunc(Func pfunc,const SQChar * name) 
    {
    ...
    template<typename Callee,typename Func>    SQClassDef & staticFunc(Callee & callee,Func pfunc,const SQChar * name) 
    {
    ...
    template<typename VarType>SQClassDef& var(VarType TClassType::* pvar,const SQChar * name,VarAccessType access=VAR_ACCESS_READ_WRITE) 
    {
        struct CV {
    ...
    //mco arr type
    template<typename VarType>SQClassDef & arr(VarType TClassType::* pvar,const SQChar * name,VarAccessType access=VAR_ACCESS_READ_WRITE) 
    {
        struct CV {
            VarType TClassType::* var;
        } cv; // Cast Variable helper.
        cv.var = pvar;
        RegisterInstanceVariableStorage(newClass,ClassType<TClassType>::type(),*(VarType **)&cv,name,access);
        return *this;
    } // var


    // Register a member variable as a UserPointer (read only).
    template<typename VarType>SQClassDef & varAsUserPointer(VarType TClassType::* pvar,const SQChar * name) 
    {
        struct CV {
            VarType TClassType::* var;
        } cv; // Cast Variable helper.
    ...
    template<typename VarType>SQClassDef & staticVar(VarType * pvar,const SQChar * name,VarAccessType access=VAR_ACCESS_READ_WRITE) 
    {
        struct CV {
            VarType * var;
        } cv; // Cast Variable helper.
...
#include "SqPlusConst.h"
    ...
    template<typename ConstantType> SQClassDef & constant(ConstantType constant,const SQChar * name) 
    {
