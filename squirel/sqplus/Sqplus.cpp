...
//=======================================================================
 ...
 //mco
  ...
  case TypeInfo<short>::TypeID: {
      short * val = (short*)data; // Address
      if (val) {
          *val = (short)sa.GetInt(3);
          return sa.Return(*val);
      } // if
      break;
                               } // case
  ...
  case VAR_TYPE_ASTRING: {

      ScriptCharArray * val = (ScriptCharArray *)data; // Address
      if (val) {
          const SQChar * strVal = sa.GetString(3);
          if (strVal) {
              *val = strVal;
...
static int getVar(StackHandler & sa,VarRef * vr,void * data) {
 ...
 case TypeInfo<short>::TypeID: {//mco
      if (!(vr->access & VAR_ACCESS_CONSTANT)) {
          short * val = (short *)data; // Address
          if (val) {
              return sa.Return(*val);
          ...
          short * val = (short *)&data; // Constant value
          return sa.Return(*val);
      } // if
      break;
                               } // case
  ...
  case VAR_TYPE_ASTRING: {
      if (!(vr->access & VAR_ACCESS_CONSTANT)) {
          ...
          throw SquirrelError(_T("getVar(): Invalid type+access: 'ScriptCharArray' with VAR_ACCESS_CONSTANT (use VAR_ACCESS_READ_ONLY instead)"));
      } // if
      break;
                                              } // case
