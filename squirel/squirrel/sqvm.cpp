...
void SQVM::ToString(const SQObjectPtr &o,SQObjectPtr &res)
{
	...
	case OT_BYTE: //mco
        {
            char l[4];
            sprintf(l,"%d",_integer(o));
		    scsprintf(_sp(rsl(6)),l);
        }
		break;
*/
    case OT_BOOL:
...
const SQChar *IdType2Name(SQObjectType type)
{
    ...
    ///::::case _RT_BYTE:return _SC("uchar");
...
bool SQVM::IsFalse(SQObjectPtr &o)
{
	if((type(o) & SQOBJECT_CANBEFALSE) && 
       ( (type(o) == OT_FLOAT) &&
       (_float(o) == SQFloat(0.0)) ) || (_integer(o) == 0) ) { //OT_NULL|OT_INTEGER|OT_BOOL
		return true;
	}
	return false;
}
...
common_call:
			...
			case _OP_SET:
                ...
                {
                    /*
                    if(STK(arg1)._type==OT_STRING && 
                       STK(arg2)._type==OT_INTEGER && 
                       STK(arg3)._type==OT_INTEGER)
                    TARGET = STK(arg);
                    else
                    */
...
bool SQVM::Set(const SQObjectPtr &self,const SQObjectPtr &key,const SQObjectPtr &val,bool fetchroot)
{
    ...
    case OT_STRING:
        if(!sq_isnumeric(key)) {Raise_Error(_SC("indexing %s with %s"),GetTypeName(self),GetTypeName(key)); return false; }
        {
            int idx = tointeger(key);
            if(idx >= _string(self)->_len )
                {Raise_Error(_SC("indexing %s with %s exceeds the length"),GetTypeName(self),GetTypeName(key)); return false; }
            return _string(self)->_val[idx] = (SQChar)val._unVal.nInteger;
        }
...
void SQVM::dumpstack(SQInteger stackbase,bool dumpall)
{
	...
	for(SQInteger i=0;i<size;i++){
        ...
        //:::case OT_BYTE:			scprintf(_SC("BYTE %d"),_integer(obj));break;//mco 
