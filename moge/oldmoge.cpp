...
#endif //
{
    ...
    if(init.Rebase() )
    {
        ...
        while(1)
        {
            ...
            while(0==pCmd)
            {
                if(!helpshown)
                {
                    printf("Run Moge with command line parameter or type in at moge prompt:\n"
                            ...
                            "-'q' to quit\n"
                            "type either: 'walker.dll' or 'bspview.gs'<cr>\n"
            ...
            printf("%s\n",uloco);
            char* pgs=0, *pdll=0;
            if((pgs=strstr(uloco,".GS")) || (pdll=strstr(uloco,".GLL")) || (pdll=strstr(uloco,".DLL")))
            {
                ...
                //
                PGs = System::Create(hInstance, pCmd);
                if(PGs)
                {
                    ...
                    else if(pdll)
                    {
                        ...
                        if(hdll > HMODULE(32))
                        {
                            ...
                            pfmain* np = (pfmain*)::GetProcAddress(hdll, _T("main"));
                            if(np)
                            {
                                (np)(PGs, hInstance, pCmd);
                            }
                            else
                            {
                                printf("%s is not as valid game dll plugin\r\n", filename);
                            }
                            ::FreeLibrary(hdll);
                        }
                        else
