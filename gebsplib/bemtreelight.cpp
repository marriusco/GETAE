...
//---------------------------------------------------------------------------------------
PointLight*	BeamTree::GetDynaLight()
{
    int i = MAX_DL;
	PointLight* pl = _lInfo._dynalights;
 	while(--i>=0 && (pl)->_flags)
	{
		pl++;
	}
	...
	if(0 > i)                             // if full return allways the first one
		pl = &_lInfo._dynalights[0];
	pl->Clear();
	pl->_flags = (LIGHT_STEADY|LIGHT_POINT);
	++_lInfo._count;
	return pl;
}

//---------------------------------------------------------------------------------------
void		BeamTree::ReleaseDynaLight(PointLight* pl)
{
	if(pl->_flags)
	{
		pl->_flags = 0;
...
//---------------------------------------------------------------------------------------
void	BeamTree::SetDynaLightAttrib(PointLight* pl,V3& pos, REAL radius, int intens, CLR& color)
{
    pl->Set(pos, radius, intens, color);
}
...
void    BeamTree::ApplyDynaLights()
{
    ...
    for(int i=0;i < MAX_DL; i++, pl++)
    {
		...
		if(!_pCamera->CanSeeCamera(pl->_pos,pl->_radius))
			continue;
	    ...
	    for(int j=1; j < _models._size; j++,pModel++)
        {
            ...
            if(!pModel->_bx.IsTouchesSphere(pl->_pos, pl->_radius))
                continue;
            ...
            if(pModel->_props & MODEL_DYNAMIC)
                pModel->TransformVertex(pl->_pos);

			R_IlumDetailModel(pModel, pl , i);

            if(pModel->_props & MODEL_DYNAMIC)
                pModel->UnTransformVertex(pl->_pos);
        }
    }
}
...
void    BeamTree::R_IlumDetailModel(BspModel* pModel, PointLight* pl, int lIdx)
{
	...
	{
	    if(!_nodes[pLeaf->_nodeIdx]._bbox.IsTouchesSphere(pl->_pos, pl->_radius))
        {
               continue;
        }
...
void    BeamTree::R_IlumMainModel(int idxNode, PointLight* pl, int lIdx)
{
    ...
    if(!node._bbox.IsTouchesSphere(pl->_pos,pl->_radius))
        return;
    if(!_pCamera->CanSee(node._bbox))
        return;

    REAL    rdist = _planes[node._planeIdx].FastDistTo(pl->_pos);
    if(rdist > pl->_radius)
    {
...
void    BeamTree::CastDynaLightInLeaf(BtLeaf* pLeaf, PointLight* pl, int lIdx)
{
    ...
    for(int j=0;j<pLeaf->_polys;j++,pFace++)
    {
        ...
        if(rPlane.GetSide(pl->_pos) < 0)            // skip back planes ////???? test if necesarely
            continue;

        //if(rPlane.DistTo(pl->_pos) > pl->_radius)
        //    continue;
  
        if(!pFace->_box.IsTouchesSphere(pl->_pos, pl->_radius*10))
        {
            continue;
        }
                              ...
                              int& blit)
{
    ...
    REAL  r2 = pl->_radius*pl->_radius*pl->_radius;
    V3*   pUV= pFace->_lmapaxes;
